...
class TpLinkOnus {
  constructor(private client: TpLinkHttpClient) {}

  async enable(serial: string): Promise<Onu> {
  ...
  async disable(serial: string): Promise<Onu> {
    const key = await this.findOnuKey(serial);
  ...
  private async findOnuKey(serial: string): Promise<string> {
  ...
  async allWithoutMac(): Promise<Onu[]> {
    const response = await this.client.post('data/PON/onuList.json', {
      operation:'load',
      port:'PON 1/0/1-8'
    });
  ...
  async macTable(): Promise<MacTableItem[]> {
  ...
  async findKeyByMacAddress(mac: string): Promise<string | null> {
    if (!isMacAddress(mac)) {
      return null
    }
